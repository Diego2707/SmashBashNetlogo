globals [ mushroom-x mushroom-y player-number game column make-state mushroom-state points coins world game-level update-points]
...
;order of breed creation matters because if two turtles are ontop of each other the one with the breed created later will appear on the top
...
players-own [move-state height-adjust level]
...
to update-output
  clear-output
  output-type "Points: " output-type points output-type "   Coins: " output-type coins output-type "x" output-type "   World " output-type world output-type "-" output-type game-level
end
...
to setup-player ;sets up the player
  create-players 1 [
    ...
    set mushroom-state "mushroom" set points 0] ;even though the shape is fixed set heading 90 is necessary for movement 
end

;individual
to setup-enemies [height x-coordinate]
  create-enemies 1 [set ycor height set xcor x-coordinate set shape "goomba" set color 48]
end
...
to setup-disguised-blocks [how-many height x-coordinate prize prize-color prize-number]
  repeat how-many [
...
to setup-flag
  ...
  create-flags 1 [set ycor height set shape "flag" set color green set xcor max-pxcor]
  create-flags 1 [set ycor height + 1 set shape "flag-top" set color green set xcor max-pxcor]
...
to move-right ;moves player right 
  if breed = players and (not any? turtles-at 1 0 or any? mushrooms-at 1 0 or any? flags-at 1 0) [ ;if there is nothing to the right of the player the player is able to move right
    if not can-move? 1 [rt 180]
    fd 1]
end

to move-left ;moves player left
  if breed = players and not (xcor = min-pxcor) and (not any? turtles-at -1 0 or any? mushrooms-at -1 0 or any? flags-at -1 0) [ ;if there is nothing to the left of the player the player is able to move left
    if not can-move? 1 [rt 180]
    bk 1]
end
...
to mushroom-test
  if breed = players and any? mushrooms-here [
    ...
    [
      set shape "supermario" set color 28 ;later replace supermario with supermario2
      set mushroom-state "fire flower"
      set level 2
    ] 
  ]
...
to enemy-kill ;player kills enemies
  if breed = players and any? enemies-at 0 (-1 * height-adjust) [ask enemies-at 0 -1 [set shape "squished goomba" wait .05 set points (points + 100) die]]
end
...
to enemy-move ;moves enemy to the right
...
to move-jump-up-part ;moves the player up until an obstacle is met (5 units)
  if breed = players and not (ycor = max-pycor - 1) [
    if not any? turtles-at 0  height-adjust  [set ycor ycor + 1]]
  
end

to move-jump-down-part ;makes sure the player falls to the ground
  if breed = players and not (ycor = min-pycor) [
    if not any? turtles-at 0 (-1 * height-adjust)  [set ycor ycor - 1] ]
end
...
to prize-condition ;turns question-blocks into used blocks if the player smashes into them from the bottom
  if (breed = question-blocks or breed = disguised-blocks) and any? players-at 0 -2 [
    ...
    flash .01 set color 23  
    if any? mushrooms-here [
    ask one-of mushrooms with [xcor = mushroom-x and ycor = mushroom-y] [set ycor ycor + 1 if shape = "coin" 
...
to setup-column-level-one
  ...
  [
    setup-disguised-blocks 1 7 max-pxcor "green mushroom" (green - 2) 1
    setup-normal-ground
    everytime 49
  ] 
  ...
  [
    setup-empty-blocks 1 10 max-pxcor
    setup-disguised-blocks 1 6 max-pxcor "coin" orange 12
    setup-normal-ground
    everytime 79
  ];hidden 12 coins
  ...
  [
    setup-question-blocks 1 10 max-pxcor "coin" orange
    setup-disguised-blocks 1 6 max-pxcor "star" orange 1
    setup-normal-ground
    everytime 86
  ] ;hidden star 
