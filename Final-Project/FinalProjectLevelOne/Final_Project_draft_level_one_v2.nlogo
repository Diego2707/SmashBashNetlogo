globals [ mushroom-x mushroom-y player-number game column make-state]
...
to setup-player ;sets up the player
  create-players 1 [set ycor min-pycor + 2.25 set xcor -15 set heading 90 set shape "mario" set color 28 set size 1.7 set player-number who] ;even though the shape is fixed set heading 90 is necessary for movement 
end
...
to mushroom-test
  if breed = players and any? mushrooms-here [
    ask mushrooms-here [die]
    flash .02
    ;set shape "supermario"
  ]
end
...
to move-jump-up-part ;moves the player up until an obstacle is met (5 units)
  if breed = players and not (ycor = max-pycor) [
    if not any? turtles-at 0 1.25  [set ycor ycor + 1]]
